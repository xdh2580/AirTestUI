...
log = get_logger("ImageLoginPage")


class ImageLoginPage(BasePage):
    """
    纯图像识别登录页面
    ...
    def __init__(self):
        ...
        # 登录页 Logo（用于验证页面是否加载）
        self.img_logo = Template(
            self.resource_path("android/img_login_logo.png"),
            threshold=0.8,
        )
    ...
    def is_login_page_loaded(self) -> bool:
        ...
        """
        log.info("验证登录页面是否已加载 (图像识别)")
        return self.is_exists(self.img_logo)

    @allure.step("等待登录页面加载")
    def wait_login_page_loaded(self, timeout=15):
        """
        等待登录页面完全加载（等待 Logo 出现）
        ...
        Returns:
            Logo 图像的坐标位置
        """
        log.info(f"等待登录页面加载, 超时: {timeout}s")
        return self.wait_for_element(self.img_logo, timeout=timeout)
    ...
    def assert_login_page_loaded(self):
        """断言登录页面已加载（严格断言，失败抛异常）"""
        self.assert_element_exists(self.img_logo, "登录页面应已加载")
